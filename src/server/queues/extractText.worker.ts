...
export const extractTextProcessor = async (job: Job<{ contractVersionId: string, buffer?: any }>) => {
  ...
  try {
    ...
    await prisma.contractVersion.update({
      ...
      const buffer = (job.data as any).buffer ? Buffer.from((job.data as any).buffer) : Buffer.from('');
      ...
      if (isDocx) {
        ...
        if (charsPerPage < 50) {
          console.log(`Detected scanned PDF for version ${contractVersionId} (${charsPerPage} chars/page). Falling back to OCR...`);
      ...
      console.error(`Error processing ContractVersion ${contractVersionId}:`, errorMessage);
...
export const extractTextWorker = connection ? new Worker('extract-text', extractTextProcessor, { connection: connection as any }) : null;
...
async function callHostedOcrApi(buffer: Buffer): Promise<string> {
  ...
  return new Promise(resolve => setTimeout(() => resolve("Extracted text from OCR API"), 500));
}
