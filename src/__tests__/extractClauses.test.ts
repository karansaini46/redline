...
describe("extractClauses service", () => {
  ...
  it("extracts and deduplicates exactly 6 clause types from a long contract with correct page numbers", async () => {
    ...
    const filler = "This is a filler text to make the contract very long. ".repeat(1000); 
    ...
    mockInvoke
      .mockResolvedValueOnce({
        clauses: [
          { clause_type: "TERMINATION", text_excerpt: "Either party may terminate this agreement with 30 days notice.", page_number: 1, char_start: 0, char_end: 50, confidence: 0.95 },
          { clause_type: "INDEMNIFICATION", text_excerpt: "The contractor shall indemnify the client against all claims.", page_number: 1, char_start: 0, char_end: 50, confidence: 0.95 },
          { clause_type: "LIABILITY_CAP", text_excerpt: "Total liability shall not exceed $10,000.", page_number: 2, char_start: 0, char_end: 50, confidence: 0.95 },
        ]
      })
      .mockResolvedValueOnce({
        clauses: [
          // This one is a duplicate (same type, high text overlap)
          { clause_type: "LIABILITY_CAP", text_excerpt: "Total liability shall not exceed $10,000.", page_number: 2, char_start: 0, char_end: 50, confidence: 0.90 },
          { clause_type: "CONFIDENTIALITY", text_excerpt: "All shared information must remain strictly confidential.", page_number: 3, char_start: 0, char_end: 50, confidence: 0.95 },
          { clause_type: "IP_ASSIGNMENT", text_excerpt: "All intellectual property developed belongs to the client.", page_number: 4, char_start: 0, char_end: 50, confidence: 0.95 },
          { clause_type: "NON_COMPETE", text_excerpt: "The contractor may not work for a direct competitor for 1 year.", page_number: 5, char_start: 0, char_end: 50, confidence: 0.95 },
        ]
      })
      .mockResolvedValue({
    ...
    const confidentialityClause = insertedData.find((d: any) => d.clause_type === "CONFIDENTIALITY");
    expect(confidentialityClause.page_number).toBe(3);

    // Check that character offsets were recalculated based on global text
    const ipClause = insertedData.find((d: any) => d.clause_type === "IP_ASSIGNMENT");
  ...
  it("retries once with a stricter prompt if malformed JSON throws an error, then throws if fails again", async () => {
    ...
    mockInvoke
      ...
      .mockResolvedValueOnce({
        clauses: [
          { clause_type: "TERMINATION", text_excerpt: "test", page_number: 1, char_start: 0, char_end: 4, confidence: 1.0 }
        ]
      });
