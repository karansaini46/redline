...
describe('Signup Transaction', () => {
  ...
  it('creates user, org, and membership atomically', async () => {
    let mockTx = {
      user: { create: vi.fn().mockResolvedValue({ id: 'user_1', email: 'test@example.com', name: 'Test' }) },
      organization: { create: vi.fn().mockResolvedValue({ id: 'org_1', name: "Test's Organization" }) },
      membership: { create: vi.fn().mockResolvedValue({ id: 'mem_1', user_id: 'user_1', org_id: 'org_1', role: 'OWNER' }) }
    }
    ...
    const result = await signup({ email: 'test@example.com', password: 'password', name: 'Test' })
  ...
  it('rolls back completely if organization creation fails', async () => {
    ...
    await expect(signup({ email: 'fail@example.com', password: 'password', name: 'Fail' })).rejects.toThrow('Database error')
