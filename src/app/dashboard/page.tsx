...
import { getClauseRiskDistribution, getExpiringContracts, getHighRiskContractTrend, getDashboardKPIs, getRecentInsights } from "./actions";
...
import { FileText, ShieldAlert, Clock, Activity, Sparkles, Download } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-6 w-full animate-fade-in">
      ...
      <FadeIn className="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-2 mb-8 border-b border-border/50 pb-6">
        <div>
          <h1 className="text-3xl font-serif font-bold tracking-tight mb-1 text-foreground">Overview</h1>
          <p className="text-sm text-muted-foreground font-light">Portfolio metrics and active processing status.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" className="bg-transparent border-border hover:bg-surface-elevated shadow-none rounded-xl gap-2">
            <Download className="size-4" />
            Report
          </Button>
          <Button size="sm" className="shadow-none bg-foreground hover:bg-foreground/90 text-background rounded-xl gap-2">
            New Contract
          </Button>
        </div>
      </FadeIn>
      ...
      <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: "Active Contracts", value: kpis.activeContracts.toLocaleString(), change: "Total", icon: FileText },
          { label: "High Risk Flags", value: kpis.highRiskFlags.toLocaleString(), change: "Attention", icon: ShieldAlert },
          { label: "Pending Renewals", value: kpis.pendingRenewals.toLocaleString(), change: "Upcoming", icon: Clock },
          { label: "Processing", value: kpis.processing.toLocaleString(), change: "Current", icon: Activity },
        ].map((kpi, i) => (
          <StaggerItem key={i}>
            <ScaleHover className="rounded-2xl border border-border/50 bg-surface shadow-sm p-6 h-full flex flex-col justify-between group hover:border-foreground/20 transition-colors">
              ...
              <div>
                ...
                <h3 className="text-3xl font-serif text-foreground">{kpi.value}</h3>
              </div>
            </ScaleHover>
          </StaggerItem>
        ))}
      ...
      <FadeIn delay={0.2} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="col-span-1 lg:col-span-2 rounded-2xl border border-border/50 bg-surface shadow-sm transition-all hover:border-foreground/20 overflow-hidden">
          <div className="flex flex-col space-y-1 p-6 border-b border-border/50 bg-muted/20">
            <h3 className="text-base font-medium font-serif tracking-tight">Clause Risk Distribution</h3>
            <p className="text-sm text-muted-foreground font-light">Risk severity across categories.</p>
          </div>
          <div className="p-6 h-[400px]">
            <Suspense fallback={<Skeleton className="h-full w-full rounded-xl bg-muted/50" />}>
        ...
        <div className="col-span-1 rounded-2xl border border-border/50 bg-surface shadow-sm transition-all flex flex-col hover:border-foreground/20 overflow-hidden">
          <div className="flex flex-col space-y-1 p-6 border-b border-border/50 bg-muted/20">
            ...
            <p className="text-sm text-muted-foreground font-light">Latest findings by Gemini.</p>
          </div>
        ...
        <div className="col-span-1 lg:col-span-2 rounded-2xl border border-border/50 bg-surface shadow-sm transition-all hover:border-foreground/20 overflow-hidden">
          <div className="flex flex-col space-y-1 p-6 border-b border-border/50 bg-muted/20">
            <h3 className="text-base font-medium font-serif tracking-tight">High-Risk Trend</h3>
            <p className="text-sm text-muted-foreground font-light">New high-risk contracts added over 90 days.</p>
          </div>
          <div className="p-6 h-[400px]">
            <Suspense fallback={<Skeleton className="h-full w-full rounded-xl bg-muted/50" />}>
        ...
        <div className="col-span-1 rounded-2xl border border-border/50 bg-surface shadow-sm transition-all flex flex-col hover:border-foreground/20 overflow-hidden">
          <div className="flex flex-col space-y-1 p-6 border-b border-border/50 bg-muted/20">
            <h3 className="text-base font-medium font-serif tracking-tight">Expirations</h3>
            <p className="text-sm text-muted-foreground font-light">Contracts expiring soon.</p>
          </div>
          <div className="p-0 flex-1 overflow-hidden h-[400px]">
            <Suspense fallback={<div className="p-6"><Skeleton className="h-full w-full rounded-xl bg-muted/50" /></div>}>
