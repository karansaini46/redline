    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center h-full min-h-[200px]">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          We haven&apos;t detected any high-risk clauses recently.
        </p>
      </div>
    );
  }
      ...
      {insights.map((insight) => {
        ...
        return (
          <StaggerItem key={insight.id}>
            <Link
              href={`/dashboard/contracts/${insight.contractId}?clause=${insight.clauseId}`}
            >
              <ScaleHover className="group flex items-start gap-4 p-3 rounded-xl hover:bg-muted/50 transition-colors cursor-pointer border border-transparent hover:border-border">
                <div
                  className={`mt-0.5 size-8 rounded-lg flex items-center justify-center shrink-0 border border-border/50 ${style.bg}`}
                >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground whitespace-nowrap">
                    There&apos;s a significant deviation in the limitation of
                    liability.{" "}
                    {formatDistanceToNow(new Date(insight.time), {
                      addSuffix: true,
                    })}
                  </span>
